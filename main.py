...
#===================Python Variables=======================
m_cat = ["Tea & Coffee","Beverages","Fast Food","South Indian","Starters","Main Course","Dessert"]

m_cat_dict = {"Tea & Coffee":"1 Tea & Coffee.txt","Beverages":"2 Beverages.txt",
...
def show_button_operation():
    ...
    if category not in m_cat:
        tmsg.showinfo("Error", "Please select valid Choice")
    else:
        menu_table.delete(*menu_table.get_children())
        f = open("Menu\\" + m_cat_dict[category] , "r")
        while True:
            line = f.readline()
            if(line==""):
                break
...
root.title("City Guest House - Food Order Management System")
...
title_frame = Frame(root, bd=8, bg="#73260E")
title_frame.pack(side=TOP, fill="x")

title_label = Label(title_frame, text="City Guest House Food Order Management System", 
                    font=("times new roman", 20, "bold"),bg = "#73260E", fg="white", pady=5)
title_label.pack()
...
#===============Menu===============
menu_frame = Frame(root,bd=8, bg="#FFFF97", relief=GROOVE)
menu_frame.place(x=0,y=125,height=585,width=680)

menu_label = Label(menu_frame, text="Menu", 
                    font=("times new roman", 20, "bold"),bg = "#FFFF97", fg="red", pady=0)
menu_label.pack(side=TOP,fill="x")

m_cat_frame = Frame(menu_frame,bg="#FFFF97",pady=10)
m_cat_frame.pack(fill="x")

combo_lable = Label(m_cat_frame,text="Select Type", 
                    font=("arial", 12, "bold"),bg = "#FFFF97", fg="blue")
combo_lable.grid(row=0,column=0,padx=10)

menuCategory = StringVar()
combo_menu = ttk.Combobox(m_cat_frame,values=m_cat,
                            textvariable=menuCategory)
combo_menu.grid(row=0,column=1,padx=30)

show_button = ttk.Button(m_cat_frame, text="Show",width=10,
                        command=show_button_operation)
show_button.grid(row=0,column=2,padx=60)

show_all_button = ttk.Button(m_cat_frame, text="Show All",
...
#===============Item Frame=============
item_frame = Frame(root,bd=8, bg="#FFFF97", relief=GROOVE)
item_frame.place(x=680,y=125,height=230,width=680)

item_title_label = Label(item_frame, text="Item", 
                    font=("times new roman", 20, "bold"),bg = "#FFFF97", fg="red")
item_title_label.pack(side=TOP,fill="x")

item_frame2 = Frame(item_frame, bg="#FFFF97")
...
item_frame3 = Frame(item_frame, bg="#FFFF97")
...
#==============Order Frame=====================
order_frame = Frame(root,bd=8, bg="#FFFF97", relief=GROOVE)
...
exit_button = ttk.Button(order_frame, text="Exit",command=exit)
exit_button.pack(side=LEFT,anchor=SW,padx=20,pady=10)
